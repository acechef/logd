...
func (l *DistributedLog) apply(requestType RequestType, message proto.Message) (interface{}, error) {
...
func (l *DistributedLog) Join(id, addr string) error {
	configuration := l.raft.GetConfiguration()
	...
	for _, server := range configuration.Configuration().Servers {
		if server.ID == serverID || server.Address == serverAddress {
			...
			removeServer := l.raft.RemoveServer(serverID, 0, 0)
			if err := removeServer.Error(); err != nil {
				return err
			}
		}
	}
	addVoter := l.raft.AddVoter(serverID, serverAddress, 0, 0)
...
func (l *logStore) StoreLogs(records []*raft.Log) error {
	for _, record := range records {
		if _, err := l.Append(&api.Record{
			Value: record.Data,
			Term:  record.Term,
			Type:  uint32(record.Type)}); err != nil {
			return err
		}
	}
	return nil
