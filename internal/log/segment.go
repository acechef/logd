...
func newSegment(dir string, baseOffset uint64, config Config) (*segment, error) {
	...
	indexFile, err := os.OpenFile(filepath.Join(dir, fmt.Sprintf("%d%s", baseOffset, ".index")),
...
func (s *segment) IsMaxed() bool {
	return s.store.size >= s.config.Segment.MaxStoreBytes || s.store.size >= s.config.Segment.MaxIndexBytes
}
